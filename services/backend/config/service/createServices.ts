...
export default async function createServices() {
    ...
    services.forEach(async (service) => {
        // recipe
        service.createRecipe(defaultRecipes[Math.floor(Math.random() * defaultRecipes.length)] as Recipe);
        // vehicle
        const vehicle = await service.createVehicle(defaultVehicles[Math.floor(Math.random() * defaultVehicles.length)] as Vehicle);

        // brand
        vehicle.createBrand(defaultVehicleBrands[Math.floor(Math.random() * defaultVehicleBrands.length)] as VehicleBrand);
        // model
        vehicle.createModel(defaultVehicleModels[Math.floor(Math.random() * defaultVehicleModels.length)] as VehicleModel);

        // client
        vehicle.createClient(defaultClients[Math.floor(Math.random() * defaultClients.length)] as Client);

        // 4 random operators
        for (let i = 0; i < 4; i++) {
            service.createOperator(defaultOperators[Math.floor(Math.random() * defaultOperators.length)] as Operator);
        }
    });
}
